...
select *
from reparto
where month(fecha_nac)='02' 
and char_length(nombre) >= (select nombre from reparto where apellido like 'Hayek') 
and apellido not like 'Hayek';
...
on r.dni=p.protagonista 
where p.nota < (select p.nota 
from reparto as r 
inner join peliculas as p on r.dni=p.protagonista 
where r.nombre 
like 'will');
...
from reparto as r 
inner join peliculas as p on r.dni=p.protagonista 
where p.nota > (select max(p.nota) 
from reparto as r inner join peliculas as p on r.dni=p.protagonista 
where r.apellido 
like 'clooney')
...
from reparto as r inner join peliculas as p on r.dni=p.protagonista 
where p.protagonista = (select r.dni 
from reparto as r inner join peliculas as p on r.dni=p.protagonista 
where p.nombre 
like 'oceans eleven');

-- Ejercicio EXTRA.- Muestra los datos de las películas con puntuación mayor que la peor película del actor que haya nacido en el mes de diciembre, y menor que la mejor película que haya rodado el mismo actor. El actor en cuestión no puede ser el mismo que haya rodado la película "Bright".
select p.* 
from reparto as r inner join peliculas as p on r.dni=p.protagonista 
where p.nota > (select min(p.nota) 
from reparto as r inner join peliculas as p on r.dni=p.protagonista  
where month(fecha_nac) ='12' and p.nombre 
not like 'bright')
and p.nota <(select mac(p.nota) 
from reparto as r inner join peliculas as p on r.dni=p.protagonista 
where month(fecha_nac)='12' and p.nombre 
not like 'bright');